...
projects = [
    {
        ...
        "description": "An end-to-end Marketing Operations platform built with React and Vite, running on a modular Supabase backend. Data is stored in PostgreSQL with structured schemas designed for scale, and access is managed through Supabase's built-in authentication layer. Key modules include an advanced system for managing influencer collaborations with automated analytics, an interactive planner for coordinating campaigns and events, and a scheduling tool for managing promotional banners with options for bulk adjustments. The interface uses a translucent, modern visual style implemented with Tailwind CSS v4 and supports both right-to-left layouts and dark mode, providing a fast and efficient workspace for managing marketing workflows.",
        "url": "https://libero-perfume.netlify.app/",
        "image_path": "images/marketing-dashboard.png"
    },
    {
        "title": "Dental Clinic Management System",
        "description": "A full-stack Dental Clinic Management System built with React and Vite and deployed on a serverless Firebase setup. Data is handled through Firestore with real-time synchronization, and user access is managed via Firebase Authentication with structured role permissions. The platform includes a custom scheduling engine that generates and adjusts appointment slots in real time, along with an automated notification module integrated through EmailJS. The interface uses a translucent, modern visual style created with Tailwind CSS and Shadcn UI and is fully adapted for right-to-left layouts, providing an efficient environment for patient interaction and clinic operations.",
